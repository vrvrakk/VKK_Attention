...
def extract_trfs(predictions_dict, stream=''):
    ...
    for sub, rows in predictions_dict.items():
        ...
        if stream == 'target':
            phoneme_idx = get_pred_idx(stream)
        else:
            phoneme_idx = get_pred_idx(stream)
...
if __name__ == '__main__':
    ...
    plane_X_folds = {cond: {} for cond in conditions}
    ...
    for condition in conditions:
        ...
        for sub, target_data, distractor_data, alpha_arr in zip(sub_list, target_dict.values(), distractor_dict.values(),
            ...
            X = X.drop(columns=([col for col in list(X.columns) if col in ['target_envelopes','distractor_envelopes'
                                                                    ,'onsets_target', 'onsets_distractor', 'alpha']]))
    ...
    X_e1 = plane_X_folds[conditions[2]]
    X_e2 = plane_X_folds[conditions[3]]
    X_folds_concat = []
    for sub_arrays1, sub_arrays2, sub_arrays3, sub_arrays4 in zip(X_a1, X_a2, X_e1, X_e2):
        print(sub_arrays1.shape)
        sub_arr_concat = np.concatenate((sub_arrays1, sub_arrays2, sub_arrays3, sub_arrays4), axis=0)
        X_folds_concat.append(sub_arr_concat)
    Y_cond1 = plane_Y_folds[conditions[0]][6:]
    Y_cond2 = plane_Y_folds[conditions[1]][6:]
    Y_cond3 = plane_Y_folds[conditions[2]]
    Y_cond4 = plane_Y_folds[conditions[3]]
    Y_folds_concat = []

    for eeg_arr1, eeg_arr2, eeg_arr3, eeg_arr4 in zip(Y_cond1, Y_cond2, Y_cond3, Y_cond4):
        sub_eeg_concat = np.concatenate((eeg_arr1, eeg_arr2, eeg_arr3, eeg_arr4), axis=0)
