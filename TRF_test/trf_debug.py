...
if __name__ == '__main__':
    ...
    s_predictors = s2_predictors  # change to s2_predictors for elevation if needed
    cond = 'a2'
    # Save R-values
    predictor_short = "_".join([p[:2] for p in pred_types])  # e.g., 'on_env_rt_ovr'
    output_dir = default_path / f'data/eeg/trf/trf_testing/composite_model/single_sub/debug/{plane}/{cond}/{folder_type}/{predictor_short}'
    ...
    for subject in subjects:
        ...
        eeg = eeg_clean_list_masked2[subject].mean(axis=0)  # average over ROI channels
    ...
    plt.title(f'TRF Composite Model: {plane.capitalize()} - {selected_stream.replace('_', ' ').capitalize()} - {folder_type.replace('_', ' ').capitalize()}')
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(output_dir / f"subjectwise_trf_{plane}_{selected_stream}_{predictor_short}.png", dpi=300)
    plt.show()
