...
def save_trf_results(sub, condition, results, type1, type2):
    ...
    results_df.to_csv(save_dir / f'{sub}_{condition}_{type1}_{type2}_trf_results.csv', index=False)
...
if __name__ == '__main__':
    ...
    for sub in subs:
        ...
        for i, predictors in enumerate(predictors_list):
            ...
            eeg_clean1, predictor_clean_onsets1 = filter_bad_segments(weight_series1[0], predictor_key=weight_keys[0])

            eeg_clean1 = eeg_clean1.T  # transpose to (74624, 5)
            eeg_clean2, predictor_clean_onsets2 = filter_bad_segments(weight_series2[1], predictor_key=weight_keys[0])
            eeg_clean2 = eeg_clean2.T  # transpose to (74624, 5)
            # --- Orthogonalize stream 2 predictor ---
            predictor_ortho_onsets2 = orthogonalize(predictor_clean_onsets2, predictor_clean_onsets1)

            predictor_trials1 = np.array_split(predictor_clean_onsets1, n_trials)
            ...
            fwd_trf1.train(predictor_clean_onsets1, eeg_clean1, fs=fs, tmin=tmin, tmax=tmax, regularization=regularization1)
            ...
            prediction1, r_fwd1 = fwd_trf1.predict(predictor_clean_onsets1, eeg_clean1)
            print(f"correlation between actual and predicted response: {r_fwd1.round(3)}")

            fwd_trf2 = TRF(direction=1)
            predictor_trials2 = np.array_split(predictor_clean_onsets2, n_trials)
            ...
            prediction2, r_fwd2 = fwd_trf2.predict(predictor_clean_onsets1, eeg_clean1)
            ...
            stream_type1 = f'{stream_name1}_{selected_predictor}'
            stream_type2 = f'{stream_name2}_{selected_predictor}'
...
# todo: stupid ass animal envelopes don't work
# todo: separate event proximity pre and post
# todo: tweak to run for all subs both TRF_predictors and TRF_test
# todo: combine all predictors -> orthogonalize and create a composite model