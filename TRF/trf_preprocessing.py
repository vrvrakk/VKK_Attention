...
eeg_events_list_copy = [(events.copy(), event_ids.copy()) for events, event_ids in eeg_events_list]
# todo: pls save the events lists with the weights included for future use: envelopes/spectrograms, overlap ratios etc
...
def create_continuous_predictor(events_array, total_samples, sfreq=125, stim_duration_sec=0.745):
    """
...
# Let’s say EEG file has N total samples (after resampling)
...
for i, eeg_file in enumerate(eeg_files_list):
    ...
    predictor_stream1 = create_continuous_predictor(stream1_events_list[0], total_samples=N)
    predictor_stream2 = create_continuous_predictor(stream2_events_list[0], total_samples=N)
...
np.savez(
    ...
    sfreq=sfreq,
    stim_duration_samples=int(0.745 * sfreq),
