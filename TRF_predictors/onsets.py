...
from TRF_predictors.config import sub, condition, \
    sfreq, stim_dur, \
    default_path, predictors_path, \
    stream1_label, stream2_label, \
    stream1_nums, stream2_nums, response_nums

from EEG.params import actual_mapping

# specify directories necessary: # due to EEG data being in the SD card
results_path = Path('D:/VKK_Attention/data/eeg/preprocessed/results')
...
def create_continuous_onsets_predictor(events_array, total_samples):
    """
    ...
    Returns:
        A 1D NumPy array of length `total_samples` with weights assigned at stim onsets.
    """
    predictor = np.zeros(total_samples)
    for event in events_array:
        onset = int(event[0])
        weight = event[1]  # here we assign an integer as a label of the stimulus category
        if onset >= total_samples:
            continue  # skip this event entirely
        predictor[onset] = weight  # Hold weight for onset timepoint
    return predictor


def filter_continuous_predictor(eeg_files_list, streams_list, sfreq=sfreq, stream_type='', event_num=None):
    """
    Turns an array of [sample_index, weight, spoken_number] into a continuous predictor.
    ...
    Returns:
        A 1D NumPy array of length `total_samples` with binary impulses.
    """
    filtered_predictors = []
        ...
        for event in events_array:
            ...
            if weight == event_num:
                if event_onset >= total_samples:
                    continue  # skip this event entirely
                filtered_predictor[event_onset] = 1  # create binary impulse predictor for chosen stimulus type
...
if __name__ == '__main__':
    ...
    from copy import deepcopy
    updated_eeg_events_list = update_eeg_events(deepcopy(eeg_events_list))
    # seems correct - assigning target / distr events
    ...
    targets_onsets = filter_continuous_predictor(eeg_files_list, target_stream_events, sfreq=sfreq, stream_type='targets', event_num=4)
    ...
    nt_target_onsets = filter_continuous_predictor(eeg_files_list, target_stream_events, sfreq=sfreq, stream_type='nt_target', event_num=2)
    ...
    distractor_onsets = filter_continuous_predictor(eeg_files_list, distractor_stream_events, sfreq=sfreq, stream_type='distractors', event_num=3)
    ...
    nt_distractor_onsets = filter_continuous_predictor(eeg_files_list, distractor_stream_events, sfreq=sfreq, stream_type='nt_distractor', event_num=1)
    ...
    deviants_onsets = filter_continuous_predictor(eeg_files_list, distractor_stream_events, sfreq=sfreq, stream_type='deviants', event_num=2)
    ...
    for i, eeg_file in enumerate(eeg_files_list):
        ...
        target_predictor_stream = (target_predictor_stream != 0).astype(int)  # convert weighted inputs into 1s

        distractor_predictor_stream = create_continuous_onsets_predictor(distractor_stream_events[i], total_samples=N)
        distractor_predictor_stream = (distractor_predictor_stream != 0).astype(int) # convert weighted inputs into 1s

        predictor_responses = create_continuous_onsets_predictor(response_events_list[i], total_samples=N)
        predictor_responses = (predictor_responses != 0).astype(int) # convert weighted inputs into 1s
